...
class Point {
    ...
    public Point() {
        this.label = new Label();  // 포함 관계로 CREATE.Label 객체 생성
    }
    ...
    public void set(int x, int y, String labelText) {
        this.x = x;
        this.y = y;
        this.label.setText(labelText);  // 포함된 CREATE.Label 객체에 텍스트 설정
    }
...
// CREATE.Label 클래스는 CREATE.Point 클래스 안에 포함될 클래스입니다.
class Label {
    private String text = "";
...
public class ColorPointEx {
    public static void main(String[] args) {
        ...
        Point p = new Point();
        p.set(1, 2, "Starting CREATE.Point"); // 오버로딩된 set 메서드 호출
        p.showPoint();  // 기본 CREATE.Point 클래스의 showPoint 호출
    }
}
